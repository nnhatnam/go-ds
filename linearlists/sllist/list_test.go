...
func TestNewList(t *testing.T) {
	...
	list = New()
	list.PushFront(1)
	if list.Front() != list.head.next {
		t.Error("Front() should equal to list.head.next")
	}
	if list.Back() != list.tail {
		t.Error("Back() should equal to list.tail")
	}

	if list.len != 1 || manualCount(list) != 1 {
		t.Errorf("expected 1, got %v", list.len)
		return
	}
	...
	list = New()
	list.appendMany(1, "a", true, 1.1, TestStruct{"test struct"})
	if list.Front() != list.head.next {
		t.Error("List with multiple elements First() should equal to list.head")
	}
	if list.Back() != list.tail {
		t.Error("List with multiple elements Last() should equal to list.tail")
	}

	if list.len != 5 || manualCount(list) != 5 {
		t.Errorf("expected 5, got %v", list.len)
		return
	}
	checkListPointers(t, list, getNodeArr(list))
}
